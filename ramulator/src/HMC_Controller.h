    ...
    void tick()
    {
        ...
        if (req == queue->q.end() || !is_ready(req)) {
          if (!no_DRAM_latency) {
            ...
            if (!victim.empty()){
                vim_non_empty++;
                if(vim_non_empty%1000000==0) printf("vim_non_empty is %lld\n",vim_non_empty);
                issue_cmd(cmd, victim);
            }
            else{
                vim_empty++;
                if(vim_empty%1000000==0) {printf("vim_empty is %lld\n",vim_empty);
                printf("vim_non_empty is %lld\n",vim_non_empty);
                }
            }
